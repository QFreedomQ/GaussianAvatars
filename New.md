...
4. **动态密度过滤**：借鉴 *Gaussian Surfels (SIGGRAPH Asia 2023)*，在 `gaussians.training_setup()` 中改写 densification 阈值，使其依赖视角覆盖度。
5. **跨身份一致性**：引入面部区域注意力（可参考 *IDE-3D*），在 `metrics.py` 前先生成区域 mask，聚焦 ROI 评估。
